...
Sandbox testing notes (TASTY_SANDBOX=true):
  ...
  - SPY and QQQ equity chains work reliably and cover the same API code paths
  - DXLink streamer works fine in sandbox (same wss endpoint as production)
  Set TASTY_SANDBOX=true to auto-switch to SPY + QQQ as sandbox-safe proxies.
"""
...
def main():
    ...
    if is_test:
        ...
        print("  DXLink streaming works normally — quotes are 15-min delayed.\n")
    else:
    ...
    # -- 2. Fetch Streamer Token LAST --
    # Do this RIGHT before connecting to the WebSocket
    ws_url, streamer_token = get_streamer_token(session)
