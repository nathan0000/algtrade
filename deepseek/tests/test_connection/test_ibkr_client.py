...
class TestIBKRClient:
    ...
    def test_connect_and_run_success(self, mock_thread, sample_config):
        """Test connect and run method - success case"""
        client = IBKRClient(sample_config.ibkr)

        # Mock the connect method of EClient
        client.connect = Mock()
        client.run = Mock()
        ...
        # Mock connection wait loop
        with patch('time.sleep', return_value=None):
            # Simulate connection becoming true after a short time
            def set_connected():
                client.connected = True
            # Use a side effect to set connected after first sleep
            time.sleep.side_effect = lambda x: set_connected()
        ...
        client.reqAccountSummary.assert_called_once_with(9001, "All", "NetLiquidation,TotalCashValue,BuyingPower")

    @patch('connection.ibkr_client.threading.Thread')
    def test_connect_and_run_failure(self, mock_thread, sample_config):
        """Test connect and run method - failure case (timeout)"""
        client = IBKRClient(sample_config.ibkr)
