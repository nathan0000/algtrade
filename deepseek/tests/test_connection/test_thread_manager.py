...
class TestThreadManager:
    ...
    def test_market_data_loop(self, mock_ibkr_client):
        """Test market data loop when within market hours"""
        tm = ThreadManager(mock_ibkr_client)
        tm.running = True

        # Mock time to be within market hours (10:00 AM)
        mock_now = datetime(2026, 3, 10, 10, 0, 0, tzinfo=pytz.UTC)  # 10:00 AM
        with patch('connection.thread_manager.datetime') as mock_datetime:
#            mock_now = Mock()
#            mock_now.time.return_value = time.struct_time((0, 0, 10, 0, 0, 0, 0, 0, 0))  # 10:00 AM
            mock_datetime.now.return_value = mock_now

            # Run one iteration (with a quick exit after loop)
            def stop_after_one():
                tm.running = False
            with patch('time.sleep', side_effect=lambda x: stop_after_one()):
                tm.market_data_loop()

            # Should request market data for SPX and VIX
            assert mock_ibkr_client.reqMktData.call_count >= 2
            # Check that contracts were created and passed
            args_list = mock_ibkr_client.reqMktData.call_args_list
            # First call: SPX
            assert args_list[0][0][1].symbol == 'SPX'
            # Second call: VIX
            assert args_list[1][0][1].symbol == 'VIX'
    ...
    def test_market_data_loop_outside_hours(self, mock_ibkr_client):
        ...
        # Mock time to be outside market hours (8:00 PM)
        with patch('connection.thread_manager.datetime') as mock_datetime:
            mock_now = Mock()
            mock_now.time.return_value = time.struct_time((0, 0, 20, 0, 0, 0, 0, 0, 0))  # 8:00 PM
            mock_datetime.now.return_value = mock_now
